...
import { Plus, Search, BookOpen, User, Trash2, X, GraduationCap } from 'lucide-react';
...
const SubjectManagement: React.FC<SubjectManagementProps> = ({ subjects, onAddSubject, onDeleteSubject }) => {
        ...
        {filteredSubjects.map(subject => (
          <div key={subject.id} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-md transition-all group relative">
            ...
            <div className="mt-6 pt-4 border-t border-slate-50 flex items-center justify-between">
              ...
              <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Teacher</p>
            </div>
          </div>
        ))}
        ...
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] w-full max-w-lg overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
            <div className="px-10 py-8 bg-slate-50/50 border-b flex justify-between items-center">
              <h3 className="text-2xl font-black text-slate-800 tracking-tight">Setup Learning Area</h3>
            ...
            <form onSubmit={handleSubmit} className="p-10 space-y-6">
              <div className="space-y-4">
                <div>
                  ...
                  <input 
                    required 
                    type="text" 
                    className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-semibold"
              ...
              <button 
                type="submit" 
                className="w-full py-5 bg-indigo-600 text-white font-black rounded-2xl shadow-xl shadow-indigo-600/20 hover:bg-indigo-700 active:scale-95 transition-all uppercase tracking-widest text-xs"
              >
                Register Subject
              </button>
            </form>
